;(function() {
    ...
    Db.prototype = {
        openDb:function() {
             ...
             * 打开本地数据库,没有则创建
             */
            this.dataBase = openDatabase('db', "1.0", "离线资源", 1024 * 1024);
            if (!this.dataBase) {
                console.log("数据库创建失败！");
            } else {
                console.log("数据库创建成功！");
        ...
        createTable:function() {
            this.dataBase.transaction(function(context) {
    ...
    var audioFactory = (function() {
        ...
        return {
            create: function() {
                ...
                return audioObj = new AudioLoad();
            }
        };
    })();
    var audioLoadManager = (function (){
        return {
            add: function(el,name) {
                var audio = audioFactory.create();
                audio.el = el;
                audio.name = name;
                audio.load();
                $(el).parent().find('.downloadBtn').unbind().on('click', function() {
                    audio.setUrl(this);
                });
                $(el).parent().find('.removeBtn').unbind().on('click', function() {
                    audio.remove(this);
                });
            }
        };
    })();
    var startLoad = function(audios) {
        for (var i = 0, audio; audio = audios[i++]; ){
            var audiosObj = audioLoadManager.add(audio, $(audio).attr('data-name'));
        };
    };
    startLoad($('.mp3'));
